...
import {  KeyboardAvoidingView, Platform, TouchableWithoutFeedback, Keyboard, ScrollView, Image, TextInput, TouchableOpacity } from "react-native";
...
const MarathonEntryFormScreen = ({navigation, route}) => {
  ...
  const { memberId, tournamentCategory, tournamentId, teamId, marathonName } = route.params;
        ...
        >
          ...
          >
            <Container>
              ...
              <ContentArea>
                <InputBox label={'참가자명'} placeholder={'이름을 입력해주세요.'} value={name} setValue={setName} isEditMode={false} />
                <InputBox label={'전화번호'} placeholder={'010-XXXX-XXXX'} value={phoneNumber} setValue={setPhoneNumber} isEditMode={false} />
                <InputBox label={'이메일'} placeholder={'email@email.com'} value={email} setValue={setEmail} isEditMode={false} />
                <InputBox label={'생년월일'} placeholder={'YYYY/MM/DD'} value={dateOfBirth} setValue={setDateOfBirth} isEditMode={false} />
