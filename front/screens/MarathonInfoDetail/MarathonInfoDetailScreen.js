...
const MarathonInfoDetailScreen = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    // isActived 초기값 확인용
    console.log('isActived 초기값 확인용', testMyInfo.bookmarkList.includes(maraName))
    ...
    const filteredTeamMembers = testUsers.filter(user => user.teamCode === myTeamCode);
    ...
    <Wrapper>
      <ScrollView>
        ...
        <MapArea>
          ...
          source={testImg} />
          <BookmarkBtnArea>
            <BookmarkBtn text={'경로 북마크'} isActivated={isActivated} toggleBookmark={toggleBookmark} />
          </BookmarkBtnArea>
        </MapArea>
            ...
            <DetailInfoView>
              ...
              {marathonCourse&&
                <LineInfoView>
                  ...
                  <LineInfoText>대회 종목: {marathonCourse.join(', ')}</LineInfoText> 
                </LineInfoView>
              }
              {marathonHost&&
                <LineInfoView>
              ...
              <TeamContainer>
                ...
                <TeamListArea>
                  {teamMemberList.map((user) => (
                    <UserInfoBox proImg={user.userProfileImg} level={user.level} name={user.userNickName} status={'show-detail'} onPress={() => console.log("사용자 디테일창")} />
                  ))
                  }
                </TeamListArea>
              </TeamContainer>
              ...
              {/* 버튼 */}
              {myMarathonInfo?.marathonList && marathonName in myMarathonInfo.marathonList ? (
                myMarathonInfo.marathonList[marathonName].teamCode ? (
                  <BtnArea>
                    <RoundBtn text={dDay === 0? '시작하기' : `D - ${dDay}`} onPress={dDay === 0? () => console.log("시작하기 버튼 누름"): handleNotStartedWarning} />
                    <View style={{ marginTop: 15, marginBottom: 30 }}>
                      {showWarning && (
                          <Text style={{ color: color.nav_orange }}>아직 마라톤을 시작할 수 없습니다.</Text>
                        )}
                    </View>
                  </BtnArea>
                ) : (
                  <BtnArea>
                    <HalfBtnContainer>
                      <BtnHalfArea>
                        <MarathonDetailRoundBtn text={'팀 생성'} onPress={createTeam} />
                      </BtnHalfArea>
                      <BtnHalfArea>
                        {/* 시작하기 눌렀을 때 어디로 넘어갈지 생각하기 */}
                        <MarathonDetailRoundBtn text={dDay === 0? '시작하기' : `D - ${dDay}`} backColor={'o_btn'} onPress={dDay === 0? () => console.log("시작하기 버튼 누름"): handleNotStartedWarning}/>
                      </BtnHalfArea>
                    </HalfBtnContainer>
                    <View style={{ marginTop: 15, marginBottom: 30 }}>
                      {showWarning && (
                ...
                <BtnArea>
                  <RoundBtn text={'참가 신청하기'} onPress={() => setShowSelectCourseModal(true)} />
                </BtnArea>
              )}
            </DetailInfoView>
          </DetailInfoArea>
      ...
      {showSelectCourseModal&& selectCourseModalContent.item.length > 0 &&
        <SelectModal isVisible={showSelectCourseModal} content={selectCourseModalContent} setRunType={setRunType} />
      }
    </Wrapper>
  )
}
